...
function fn_kuroneko_webcollect_get_orders(&$params, &$fields, &$sortings, &$condition, &$join, &$group)
{
    ...
    ///////////////////////////////////////////////
    // 出荷登録画面の場合
    if( $controller == 'krnkwc_cc_shipments' ) {
        // 配送方法を抽出
 ...
 * @return bool
 */
function fn_krnkwc_set_err_msg($result, $title ='')
{
    ...
    fn_set_notification('E', $title, fn_krnkwc_get_err_msg($error_code, $credit_error_code) . ' (' . $error_code . ')');
}
 ...
 * @return string
 */
function fn_krnkwc_get_err_msg($error_code, $credit_error_code)
{
    // エラーコードを大文字に変換
...
///////////////////////////////////////////////
{
    ...
    if (!empty($result)) {
        ...
        // 出荷情報登録でエラーが発生した場合
            ...
            fn_krnkwc_set_err_msg($result, __('jp_kuroneko_webcollect_shipment_entry_error'));
            $is_registered = false;
        }
...
function fn_krnkwc_send_cc_request( $order_id, $type = 'creditcancel')
{
    ...
    // リクエスト送信が失敗した場合
        ...
        // エラーメッセージを表示
        fn_set_notification('E', $error_title, __('jp_kuroneko_webcollect_cc_status_change_failed'));
    }
