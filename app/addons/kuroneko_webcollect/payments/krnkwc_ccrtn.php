...
if( ($mode == 'place_order' || $mode == 'process' || $mode == 'repay') && (AREA == 'C' || (AREA == 'A' && Registry::get('runtime.action') != 'save')) ){
    ...
    if(empty($order_info['payment_info']['errorCd'])) {
        ...
        if( !empty($_REQUEST['3D_TRAN_ID']) && !empty($_REQUEST['COMP_CD']) && $_REQUEST['pt'] == '3ds' ){
            ...
            ///////////////////////////////////////////////////////////////////////////
            $tds_params = array();
            $tds_params['order_no'] = Tygh::$app['session']['krnkwc_ccrtn'][$order_id]['order_no'];
            ...
            $tds_params['three_d_token'] = Tygh::$app['session']['krnkwc_ccrtn'][$order_id]['three_d_token'];
            ///////////////////////////////////////////////////////////////////////////
            // クレジット決済登録（3Dセキュア後）に使用するパラメータをセット EOF
            ///////////////////////////////////////////////////////////////////////////

            // 受付番号と3Dトークンを保存するセッション変数をクリア
            unset(Tygh::$app['session']['krnkwc_ccrtn']);
        ...
        if (!empty($cc_result)) {
            ...
            if ( $return_code == 0 ) {
                ...
                if( !empty($cc_result['threeDAuthHtml']) ){
                    ...
                    if( !empty($cc_result['threeDToken']) ) Tygh::$app['session']['krnkwc_ccrtn'][$order_id]['three_d_token'] = $cc_result['threeDToken'];
                    if( !empty($params['order_no']) ) Tygh::$app['session']['krnkwc_ccrtn'][$order_id]['order_no'] = $params['order_no'];
