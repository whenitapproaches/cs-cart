                ...
                foreach ($cart['product_groups'] as $group_key => $group) {
                    if (!empty($group['chosen_shippings'])) {
                        foreach ($group['chosen_shippings'] as $shipping_key => $shipping) {
                            ...
                            // お届け日が指定された場合
                            if( isset($_REQUEST['delivery_date_selected_' . $group_key . '_' . $delivery_shipping_id]) ){
                                // カート情報にお届け時間帯をセット
                                $cart['shipping'][$delivery_shipping_id]['delivery_info'][$group_key]['delivery_date_selected'] = $_REQUEST['delivery_date_selected_' . $group_key . '_' . $delivery_shipping_id];
                            }

                            // お届け時間帯が指定された場合
                            if( isset($_REQUEST['delivery_time_selected_' . $group_key . '_' . $delivery_shipping_id]) ){
                                // カート情報にお届け時間帯をセット
                                $cart['shipping'][$delivery_shipping_id]['delivery_info'][$group_key]['delivery_time_selected'] = $_REQUEST['delivery_time_selected_' . $group_key . '_' . $delivery_shipping_id];
                            }
                        }
                    }
                }
...
// （氏名などが登録されていないため）
function fn_localization_jp_update_user_pre(&$user_id, &$user_data, &$auth, &$ship_to_another, &$notify_user)
{
    // モードを取得
    $_mode = Registry::get('runtime.mode');
...
function fn_localization_jp_get_products_pre(&$params, &$items_per_page, &$lang_code)
{
    ...
    if(Registry::get('addons.localization_jp.jp_prod_search_mode') == "jp_prod_srch_all"){
        $match = "all";
    }
    else {
        $match = "any";
 ...
 * @return string
 */
function fn_lcjp_get_vendor_invitation_mtpl_url()
{
    $tpl_id = db_get_field("SELECT tpl_id FROM ?:jp_mtpl WHERE tpl_code = ?s", "vendor_invitation");

    // メールテンプレートアドオンが有効な場合
    if ( Registry::get('addons.mail_tpl_jp.status') == 'A' ) {
        $url = "mail_tpl_jp.update&tpl_id=" . $tpl_id;
    }
    else {
        $url = "email_templates.update?code=vendor_invitation&area=A";
    }
