...
function fn_ap_sbps_get_payments_post($params, &$payments)
{
    ...
    if (AREA == 'C') {
        ...
        if (fn_allowed_for('MULTIVENDOR')) {
            $delete_processor_ids = array_merge($delete_processor_ids, ['9240', '9244', '9245', '9246']);
        }
        ...
        if (Registry::get('runtime.mode') === 'add') {
            $delete_processor_ids = ['9240', '9241', '9242', '9243', '9244', '9245', '9246'];
            ...
            if (!empty($payment_id)) {
                $payments = array_filter($payments, function ($payment) use ($payment_id) {
                    return $payment['payment_id'] === $payment_id || !in_array($payment['processor_id'], ['9240', '9241', '9242', '9243', '9244', '9245', '9246'], true);
                });
            }
        }
    }
...
function fn_ap_sbps_get_pay_method($order_id, $process = '')
{
    ...
    if (!empty($process)) {
        $table = "sbps_{$process}_payment_info";
        $result = db_get_field("SELECT pay_method FROM ?:{$table} WHERE order_id = ?i", $order_id);
    }
...
function fn_ap_sbps_set_order_data($order_id, $attr, $process)
{
    ...
    // 不要な情報を削除
    foreach (['order_id', 'order_status', 'recognized_no', 'commit_status', 'refund_rowno', 'bonus_details_times'] as $key) {
    ...
    foreach ($attr as $key => $value) {
        ...
        switch ($key) {
            ...
            case 'payment_status':
                if ($process === 'offline') {
                    $format_attr[$format_key] = __("sbps_offline_payment_status_{$value}");
                } else {
