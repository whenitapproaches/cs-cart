...
class Sbps
{
    ...
    public function valid_mode_exec($order_id, $mode, $process)
    {
        ...
        if (!empty($payment_info)) {
            // ステータス毎の可能な処理チェック
            if (!empty(SBPS_EXEC_PERMISSION[$payment_info['pay_method']]['status'][$payment_info['payment_status']])) {
                $status_exec_permission = array_map(function ($mode){
                    return trim($mode, '_');
                }, SBPS_EXEC_PERMISSION[$payment_info['pay_method']]['status'][$payment_info['payment_status']]);

                $result = in_array($mode, $status_exec_permission, true);
            }

            // 全ステータスで可能な処理チェック
            if (!$result && !empty(SBPS_EXEC_PERMISSION[$payment_info['pay_method']]['all'])) {
                $all_exec_permission = array_map(function ($mode){
                    return trim($mode, '_');
                }, SBPS_EXEC_PERMISSION[$payment_info['pay_method']]['all']);
    ...
    protected function format_item_name($products)
    {
        ...
        $item_name = count($products) == 1 ? $product['product'] : mb_strcut($product['product'], 0, 36, 'UTF-8') . ' ' . __('jp_sbps_etc');
    ...
    protected function create_request_data($data, $encode = true)
    {
        ...
        array_walk_recursive($data, function (&$value) use (&$sps_hashcode, $encode) {
            if (!is_array($value)) {
                $value = fn_lcjp_replace_pdc($value);

                if ($encode) {
                    $value = mb_convert_encoding(fn_lcjp_replace_pdc($value), 'Shift_JIS', 'UTF-8');
                }
