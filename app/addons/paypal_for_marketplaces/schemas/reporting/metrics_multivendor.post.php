...
/**
 * @return bool Whether PayPal for Marketplaces is used
 */
$schema['paypal_for_marketplaces'] = function () {
    $processor_data = fn_get_processor_data_by_name('paypal_for_marketplaces.php');
    ...
    if ($processor_data) {
        ...
        foreach ($payment_ids as $payment_id) {
            ...
            if (!empty($data['processor_params']['client_id'])
                ...
                && !empty($data['processor_params']['bn_code'])
            ) {
                return true;
            }
        }
