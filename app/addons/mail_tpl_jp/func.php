	...
	if( !empty($items_ordered) ){
		...
		foreach($items_ordered as $k => $v){
            ...
            if( $v > 0 && ( $type == 'NORMAL' || ($type == 'SUPPLIERS' && in_array($k, $supplier_products)) ) ){
                $cnt++;
                $ordered_products .= SecurityHelper::escapeHtml($v['product'], true);
...
function fn_mtpl_get_profiles_extra_fields_by_order($fields, $order_info = array())
{
	...
	foreach($fields as $field){
		...
		// classを持たないフィールドをユーザー追加フィールドと見なす
		if(!$field['class'] || $field['class'] == 'first-name-kana' || $field['class'] == 'last-name-kana'){
...
function fn_mtpl_get_profiles_extra_fields($tpl_user_data, $section)
{
	...
	if(!$tpl_user_data){
		...
		// ユーザー追加フィールドのIDを取得
		$extra_field_ids = db_get_fields("SELECT field_id FROM ?:profile_fields WHERE (class = '' OR class IN ('first-name-kana', 'last-name-kana')) AND section = ?s", $section);
