...
<div class="table-responsive-wrapper">
    ...
    <thead>
        <tr>
            <th class="left mobile-hide" width="5%">{include file="common/check_items.tpl"}</th>
    ...
    <tr>
        ...
        <td data-th="{__("tools")}">
            ...
            {if $product.approved == "Y" || $product.approved == "P"}
                {include file="common/popupbox.tpl" id="disapprove_`$product.product_id`" text="{__("disapprove")} \"`$product.product`\"" content=$smarty.capture.disapprove act="notes" icon="icon-thumbs-down"}
            {/if}
            
            {if $product.approved == "P" || $product.approved == "N"}
                {include file="common/popupbox.tpl" id="approve_`$product.product_id`" text="{__("approve")} \"`$product.product`\"" content=$smarty.capture.approve act="notes" icon="icon-thumbs-up"}
            {/if}
    ...
    {capture name="buttons"}
        {capture name="tools_items"}
            {hook name="companies:manage_tools_list"}
                <li>{btn type="list" text=__("disapprove_selected") form="manage_products_form" class="cm-process-items cm-dialog-opener cm-dialog-auto-size" data=["data-ca-target-id" => "content_disapprove_selected"]}</li>                    
            {/hook}
        {/capture}
        {dropdown content=$smarty.capture.tools_items}

        {include file="buttons/button.tpl" but_role="submit-link" but_target_id="content_approve_selected" but_text=__("approve_selected") but_meta="cm-process-items cm-dialog-opener cm-dialog-auto-size" but_target_form="manage_products_form"}
    {/capture}
{/if}
