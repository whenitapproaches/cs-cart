                ...
                <table class="table table-middle table-last-td-align-right">
                    ...
                    {foreach from=$orders.0 item="order"}
                        <tr>
                            ...
                            <td><a href="{"orders.details?order_id=`$order.order_id`"|fn_url}">{__("order")} <bdi>#{$order.order_id}</bdi></a> {__("by")} {if $order.user_id}<a href="{"profiles.update?user_id=`$order.user_id`"|fn_url}">{/if}{if $smarty.const.CART_LANGUAGE == 'ja'}{$order.firstname} {$order.lastname}{else}{$order.lastname} {$order.firstname}{/if}{if $order.user_id}</a>{/if}</td>
